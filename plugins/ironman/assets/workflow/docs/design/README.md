# docs/design/ — Phase 1 concept state and non-contract design docs
...
## 파일 형식 예시 (`docs/design/checklist.md`)
...
계약은 아니지만 구현에 필요한 설계 산출물도 여기에 둔다. `docs/spec/`와 달리 **권위가 없다**. 구현은 참고하되
충돌 시 `docs/spec/`가 이긴다.
...
- 안정 인터페이스(스키마·API·공개 타입)는 여기가 아니라 `docs/spec/`에 둔다.
