...
> 규칙 권위는 `docs/design/AGENTS.md`(Phase 1) / `goals/AGENTS.md` §Gate validity — **봉인 시작 전 반드시 읽는다**. 체크리스트 규약은 `docs/design/README.md`.
> 이 스킬은 그 종료를 **봉인**하는 오케스트레이터다. 새로 만드는 본체는 ③ spec 증분 확장 하나뿐 —
> 나머지는 기존 스킬(to-prd·to-spec·to-issues)을 sealed로 수행한다.

## 진입 전제
- 대상 기능의 `docs/design/checklist.md`가 **닫혀 있어야** 한다(미표기 `[ ]` 0개 **그리고**
...
4렌즈·근거 전문은 `docs/design/AGENTS.md` §Pre-freeze intent-audit(**봉인 전 필독**).
...
## 완료
- `docs/design/checklist.md`의 그 기능을 **`FROZEN (날짜, freeze 완료 → issues NNN~MMM)`**로 표기.
...
- 동결 결정의 **근본 오류** → 사용자 승인 후 `concept` 재오픈(Phase 1 재진입). 라우팅: 단순 추가/충돌 → findings,
  동결 결정 자체가 틀림 → concept 재오픈. (`docs/design/AGENTS.md`)
