# goals/EXAMPLE.md — 변환 정답지 (issue → 3파일 goal 계약)

> **이 파일은 goal이 아니라 문서다.** discovery rule(이름이 숫자로 시작하거나
> `_meta.md`인 파일만 goal로 인식 — `goals/AGENTS.md`)이 무시하므로 체인·게이트에
> 영향이 없고, `0-example`과 달리 첫 변환 후에도 **영구히 남는다**.
>
> **역할 분담** — `goals/0-example.*` = *실행되는* 교육용 placeholder(새 템플릿에서
> 체인이 green 되는 걸 관찰; 첫 모드 B 변환이 삭제). 이 문서 = *변환 품질의 기준*
> (모드 B 변환 전, finding을 goal로 promote하기 전에 1회 참조).
>
> 규약 본체는 `goals/AGENTS.md` · `docs/goal-design.md` §1·§1.5 — 이 문서는 예시일
...
## 입력 — 가상의 이슈 `docs/issues/003-cancel-booking.md`
...
## 출력 1 — `goals/3-cancel-booking.md` (미션)
...
취소 규칙이 계약(docs/spec/api-contract.md §cancel)대로 강제된다:
confirmed 상태에서만 취소되고, 시작 시각이 지난 예약은 409로 거부되며,
취소된 예약의 슬롯은 검색에 재노출된다.

## Completion Conditions

1. Every cancellation rule in `docs/spec/api-contract.md` §cancel
...
- 규칙 집합: `grep -oE 'CANCEL-R[0-9]+' docs/spec/api-contract.md`
- 상태 집합: `docs/spec/domain-types.md` BookingStatus enum

## Verification

    bash goals/3-cancel-booking.gates.sh
...
- **`risk: RISKY`는 이슈 frontmatter에서 그대로 운반**됐다(엣지·검증 규칙을
  ...
  (`goals/AGENTS.md` §Risk tier). 이슈에 risk가 없으면 변환 시 휴리스틱으로
...
## 출력 2 — `goals/3-cancel-booking.gates.sh` (기계 검증, chmod +x)
...
GATE_INPUTS=(
  ...
  tests/booking
  goals/3-cancel-booking.gates.sh
  goals/3-cancel-booking.md
...
done < <(grep -oE 'CANCEL-R[0-9]+' docs/spec/api-contract.md | sort -u)
...
echo "[3.3] gate rigor"
if bash "$ROOT/scripts/check-gate-rigor.sh" "$ROOT/goals/3-cancel-booking.md" >/dev/null 2>&1; then
  echo "    [PASS]"
else
  echo "    [FAIL]"
  bash "$ROOT/scripts/check-gate-rigor.sh" "$ROOT/goals/3-cancel-booking.md" | sed 's/^/      /'
  PASS=false
fi
...
- **게이트 ≠ convention police.** "취소 로직이 옳은가"는 테스트가 검증한다 —
  ...
  테스트 소유, gate에서 빼라 (`docs/goal-design.md` §1.5).
...
## 출력 3 — `goals/3-cancel-booking.next-task.sh` (힌트, chmod +x)
...
if ! compgen -G "tests/booking/*cancel*" >/dev/null; then
  echo "TASK: RED -- write the first failing cancellation test (spec: docs/spec/api-contract.md #cancel)"
elif ! bash "$ROOT/goals/3-cancel-booking.gates.sh" >/dev/null 2>&1; then
  echo "TASK: GREEN -- make the remaining cancellation rules pass, then re-run the gate"
else
  echo "TASK: goal 3-cancel-booking is green -- run scripts/active-check.sh to advance"
fi
```
...
(`guidelines/goal-iteration.md` §Designing next-task hints).
