...
- ironman structure exists. If `scripts/diagnose.sh` or `goals/AGENTS.md` is missing, run
  `/ironman:init` first.
...
3. **GENERATE.** Update only the lean FCG harness:
   ...
   - seed accepted tradeoff ADRs in `docs/adr/`
...
Use `skills/concept/CONTEXT-FORMAT.md` and `skills/concept/ADR-FORMAT.md` for formatting. Do not create a
