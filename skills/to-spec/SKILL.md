...
1. **결정표면 회계.** 엔티티 목록을 만들고 4렌즈(구조: cardinality·identity / 행위: lifecycle·동시성·엣지 /
   ...
   미결정 슬롯은 가드레일대로 `TODO(미결정)`로 두고 사용자에게 올린다.
...
4. **INDEX.md 생성/갱신.** 만든 계약문서를 `docs/spec/INDEX.md` 표에 등록한다(형식 아래). 단,
   `TODO(미결정)` 칸이 남은 계약문서는 Covers 끝에 **`(미결정 칸 있음 — 동결 전)`**을 붙여 등록하고
   frontmatter `resolved: false`를 유지한다 — 구현이 미결정 placeholder를 동결 계약으로 오독하지 않게.
...
- **핵심 기술 선택이 비어 있으면 계약을 고정하지 않는다.** 저장 방식·전달 형태(서비스/라이브러리/CLI/UI)·
  ...
  `TODO(미결정)`로 두고 **사용자에게 명시적으로 올려** grill-with-docs로 보완을 권한다. 조용히 기본값을
