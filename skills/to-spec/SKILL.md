...
- `/to-issues`·`/build` 변환·`/handoff` **전**이 정위치. 구현 모델이 `docs/spec/INDEX.md`를 최우선으로 읽기 때문.
...
> `docs/design/architecture.md`, 대시보드가 **파싱하는 서비스 운영 토폴로지**(배포 흐름)=계약 →
> `docs/spec/service-flow.md`. 전문은 `docs/design/README.md` + `docs/spec/README.md` 참조.
...
1. **결정표면 회계.** 엔티티 목록을 만들고 4렌즈(구조: cardinality·identity / 행위: lifecycle·동시성·엣지 /
   ...
   모든 load-bearing 슬롯이 `결정됨(grounded)`/`질문해 답받음`/`튜닝값 보류(표기)` 중 하나가 아니면 동결하지 않는다
   (미결정 슬롯 처리는 아래 §가드레일 단일 규칙을 따른다).
2. **독립 의도-감사 1회.** 계획을 작성하지 않은 서브에이전트에게 대화·ADR·PRD·결정표면을 읽혀
   "조용히 채워진 추측"을 사냥시킨다. 발견은 문서에 몰래 패치하지 않고 **사용자 질문으로 종결**한다.
   서브에이전트 도구가 없으면 명시적 자기-감사 패스로 대체하고 목록을 사용자에게 보인다.

> 이 게이트의 존재가 얕은 grilling의 면죄부가 되면 안 된다 — 여기서 뭔가 나오면 대화를 너무 일찍 닫은 것.
...
4. **INDEX.md 생성/갱신.** 만든 계약문서를 `docs/spec/INDEX.md` 표에 등록한다(형식·`TODO(미결정)` 칸
   표기 규칙은 아래 §`docs/spec/INDEX.md` 형식 — 구현이 미결정 placeholder를 동결 계약으로 오독하지 않게).
5. **비계약 분류.** 흐름·화면·아키텍처 내용이 입력에 섞여 있으면 `docs/design/`의 해당 문서로 옮기거나 가리키게 한다.
6. **요약 보고.** 만든 계약문서, design으로 분류한 것, **결정이 안 나 비워둔 칸**(→ grill로 보완 권유)을 보고한다.

## 계약문서 품질 floor (칸 채우기 ≠ 계약)

각 `docs/spec/*.md`에서 **내용이 채워진 항목**은 아래를 통과해야 한다. 가드레일대로 정당하게
...
- **있을 자리값(consequence-of-absence)** — 한 줄을 지웠을 때 무엇이 깨지는지 답할 수 없으면, 그 줄은 계약이 아니다(빼거나 `docs/design/`으로).
...
- **핵심 기술 선택이 비어 있으면 계약을 고정하지 않는다.** 저장 방식·전달 형태(서비스/라이브러리/CLI/UI)·
  ...
  `TODO(미결정)`로 두고 **사용자에게 명시적으로 올려** grill로 보완을 권한다. 조용히 기본값을
...
- **코드를 만들거나 고치지 않는다.** 구현은 `/build`(TDD) 영역. 이 스킬의 쓰기 대상은 `docs/spec/`와 `docs/design/`(분류 이동분)뿐.
