...
# in the template's workflow-manifest.txt, read at the template's HEAD --
# committed state only; uncommitted template edits never propagate).
# Project content (docs bodies, numbered goals, cycles history, CONTEXT.md,
# docs/state/*, scripts/smoke.sh) is NEVER touched.
#
# 3-way logic, keyed on the .workflow-version marker (template SHA this
...
#   bash scripts/update-workflow.sh --from <template-path-or-git-url>   # dry-run report
#   bash scripts/update-workflow.sh --apply                             # apply NEW+UPDATE
#   bash scripts/update-workflow.sh --set-baseline                      # mark reconciled
...
#   cd <project> && bash <template>/scripts/update-workflow.sh --from <template>
...
# -- project-root guard ------------------------------------------------------
if [ ! -f "$ROOT/AGENTS.md" ] && [ ! -d "$ROOT/goals" ]; then
  echo "[FAIL] run from the project root (no AGENTS.md / goals/ here): $ROOT"
  exit 2
fi
...
if [ -z "$SRC" ]; then
  echo "[FAIL] no template source. Pass --from <path|git-url> (it is remembered in .workflow-version)."
  exit 2
fi
...
if [ "$APPLY" -eq 1 ] && [ "$FORCE_DIRTY" -ne 1 ] && [ -n "$proj_top" ]; then
  # .workflow-version is this tool's own bookkeeping file -- an uncommitted
  # marker (e.g. right after --set-baseline) must not trip the guard.
  if [ -n "$(git -C "$ROOT" status --porcelain 2>/dev/null | grep -vE '^.. \.workflow-version$')" ]; then
...
manifest_raw=$(git -C "$TPL" show "$TPL_HEAD:workflow-manifest.txt" 2>/dev/null) || {
  echo "[FAIL] template HEAD carries no workflow-manifest.txt -- commit it in the template first."
  exit 2
}
...
if [ "$HAVE_BASE" -eq 1 ]; then
  echo "  baseline: ${BASE_SHA:0:12} (.workflow-version)"
else
...
if [ "$pending" -gt 0 ]; then
  ...
  echo "  When everything is reconciled:  bash scripts/update-workflow.sh --set-baseline"
fi
...
if [ "$APPLY" -eq 1 ]; then
  ...
  if [ "$pending" -eq 0 ]; then
    ...
    if [ ! -f "$MARKER" ]; then
      ...
      printf 'source=%s\n' "$SRC" > "$MARKER"
      echo "[..] template source remembered in .workflow-version (sha pending reconcile)"
    fi
