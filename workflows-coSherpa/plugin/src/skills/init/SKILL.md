...
Do not build a custom `PATH=...:$PATH` command around it. If `cosherpa-init` is not on PATH,
say that the plugin bin was not exposed by the host and ask the user to reinstall or restart the
host after installing the plugin. Do not hand-copy files.
