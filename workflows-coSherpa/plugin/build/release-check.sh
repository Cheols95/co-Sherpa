...
smoke_scratch_init() {
  ...
  [ -f "$project/workflows-coSherpa/scripts/diagnose.sh" ] || { echo "scratch init did not create diagnose.sh"; return 1; }
  [ -f "$project/workflows-coSherpa/.cosherpa/status" ] || { echo "scratch init did not write status"; return 1; }
  [ ! -d "$project/workflows" ] || { echo "scratch init created legacy workflows directory"; return 1; }
}
...
audit_release_metadata() {
  local claude_version codex_version init_version bad_file stale_repo_name old_plugin_name old_harness_dir
  ...
  old_harness_dir="$(printf '%s/' workflows)"
  : > "$bad_file"
  ...
  if grep -R "$old_harness_dir" \
    ...
    "$PLUGIN_ROOT/src" \
    "$PLUGIN_ROOT/dist" >/dev/null 2>&1; then
    echo "legacy harness path remains in release surface: $old_harness_dir" >> "$bad_file"
  fi
