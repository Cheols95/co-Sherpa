...
여기 **안** 들어가는 것 = 흐름·화면, **코드 아키텍처**(모듈 내부 구조) 같은 설계 산출물 → `workflows-coSherpa/docs/design/`. 결정의 "왜" → `workflows-coSherpa/docs/adr/`.

> 주의: 대시보드가 **파싱하는 서비스 운영 토폴로지**(클라우드·배포 흐름)는 설계가 아니라 계약 → 여기 `service-flow.md`.
> `workflows-coSherpa/docs/design/architecture.md`(코드 모듈 구조)와는 이름만 비슷한 **다른 문서**다.
