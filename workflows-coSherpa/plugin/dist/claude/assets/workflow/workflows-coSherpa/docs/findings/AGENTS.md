# `workflows-coSherpa/docs/findings/` — Working Protocol
...
```
workflows-coSherpa/docs/findings/<UTC-created-at>-<slug>.md
```
...
| `was`          | repo-rooted path          | no       | —       | Previous goal path (`workflows-coSherpa/goals/<n>-<name>.md`) when a promoted goal dissolved back into the queue.                         |
...
priority: P2
related:
  - workflows-coSherpa/docs/<other-doc>.md
---
# <Same as title>
```
...
related:
  - workflows-coSherpa/goals/<n>-<name>.md
---
```
...
3. **Promote** — when a finding warrants its own goal, author
   `workflows-coSherpa/goals/<n>-<name>.md` and have its `## Why This Goal Exists` section
...
5. **Archive** — append-only logs (`kind: append-only-log`) that exceed
   ...
   `workflows-coSherpa/docs/archive/findings/<slug>-<YYYY>-Q<n>.md`.
...
(`workflows-coSherpa/docs/findings/<file>.md`, `workflows-coSherpa/goals/<n>-<name>.md`, `src/...`) — not `./`
...
(`workflows-coSherpa/docs/findings/<file>.md`). Bare globs (`workflows-coSherpa/docs/findings/*.md`) are reserved
