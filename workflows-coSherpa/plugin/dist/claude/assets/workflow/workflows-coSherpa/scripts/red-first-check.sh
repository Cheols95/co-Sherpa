...
RED_FIRST_RECORD_DIR="${RED_FIRST_RECORD_DIR:-$ROOT/.state/red-first}"
...
check_dir() {
  ...
  if [ -n "$green" ]; then
    ...
    verdict="FAIL"
    mkdir -p "$RED_FIRST_RECORD_DIR" 2>/dev/null || true
    record="$RED_FIRST_RECORD_DIR/$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || echo unknown)-fail.log"
    {
      echo "verdict=$verdict"
      echo "goals_dir=$gdir"
      echo "checked_goals=$n"
      printf '%s' "$green"
  ...
  verdict="OK"
  mkdir -p "$RED_FIRST_RECORD_DIR" 2>/dev/null || true
  record="$RED_FIRST_RECORD_DIR/$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || echo unknown)-ok.log"
  {
    echo "verdict=$verdict"
    echo "goals_dir=$gdir"
    echo "checked_goals=$n"
  ...
  [ -f "$record" ] && echo "red-first-check: recorded conversion-time evidence -> ${record#"$ROOT/"}"
  return 0
}
...
self_test() {
  ...
  RED_FIRST_RECORD_DIR="$tmp/red-first-records"
