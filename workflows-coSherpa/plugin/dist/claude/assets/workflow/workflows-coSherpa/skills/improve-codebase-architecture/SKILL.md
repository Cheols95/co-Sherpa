...
description: "코드베이스의 deepening(얕은 모듈을 깊게 만들기) 기회를 CONTEXT.md의 도메인 언어와 workflows-coSherpa/docs/adr/의 결정에 비추어 찾는다. 아키텍처 개선, 리팩토링 기회 발굴, 강결합 모듈 통합, 또는 코드베이스를 더 테스트 가능·AI-탐색 가능하게 만들고 싶을 때 활성화."
---
...
사용자가 후보를 고르면 concept 대화로 들어간다. 함께 설계 트리를 걷는다 — 제약, 의존성, 깊어진 모듈의
...
- **깊어진 모듈을 `CONTEXT.md`에 없는 개념으로 명명?** 그 용어를 `CONTEXT.md`에 추가한다 — `/concept`과
  같은 규율([CONTEXT-FORMAT.md](../concept/CONTEXT-FORMAT.md) 참조). 파일이 없으면 lazily 생성.
...
- **사용자가 load-bearing 근거로 후보를 거부?** ADR을 제안하되 이렇게 틀 짓는다: _"미래 아키텍처 리뷰가
  ...
  때만 제안하라 — 일시적("지금은 가치 없음")이거나 자명한 이유는 건너뛴다. [ADR-FORMAT.md](../concept/ADR-FORMAT.md) 참조.
