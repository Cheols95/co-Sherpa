...
`completion-check.sh` recognizes `workflows-coSherpa/goals/_meta.md` and launches it first.
If it fails, `workflows-coSherpa/.state/active-goal` records `_meta` and `next-task.sh`
dispatches to `workflows-coSherpa/goals/_meta.next-task.sh`.

## The Goal (adapt to your stack)

The following hold for the whole repo. Edit `META_CHECKS` in
`workflows-coSherpa/goals/_meta.gates.sh` to match your toolchain:
...
When the project grows a runnable service, create **`workflows-coSherpa/scripts/smoke.sh`**:
...
No `workflows-coSherpa/scripts/smoke.sh` = an explicit "this project has no runtime
component" decision — record that in `workflows-coSherpa/docs/spec/`, don't leave it implicit.
