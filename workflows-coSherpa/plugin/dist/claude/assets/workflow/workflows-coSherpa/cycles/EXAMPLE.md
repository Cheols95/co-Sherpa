...
> Real cycles are named `workflows-coSherpa/cycles/<YYMMDD>-<NN>-<slug>.md`. Author yours with
> `workflows-coSherpa/prompts/cycle-generate.md` and delete this file.

**목표**: 2026-05-26 시점 `workflows-coSherpa/docs/findings/` 의 미해결(`false`) + 부분
해결(`partial`) finding 을 우선순위/의존성 순서대로 닫는다. 일부는 goal
promote, 대부분 직접 작업. 매 작업 단위 완료 시 commit + push.

이 문서를 에이전트에게 **무한 루프 모드**로 넘긴다:
`workflows-coSherpa/cycles/<this-file>.md 의 내용을 모두 완수할 때까지 작업해줘.`

**시작 상태**: chain GREEN (`workflows-coSherpa/.state/active-goal == ALL_DONE`), 최고 goal
...
LOOP:
  step 1 — chain 상태:  bash workflows-coSherpa/scripts/completion-check.sh
    exit 0 (ALL_DONE)  → step 2
    else               → workflows-coSherpa/.state/active-goal 의 goal 을 TDD 로 GREEN (goal-iteration.md Phase 4)
                         → commit + push → step 1
...
**막혀도 종료하지 마라.** stuck → 사다리(맥락 보강 → 접근 전환 → blocker
기록 → 다음 target; 단계·횟수는 goal-iteration.md §When You Are Stuck).
...
1. `workflows-coSherpa/docs/findings/<UTC>-login-rate-limit.md` (P1) — `/login` rate-limit.
   직접 작업. Acceptance signal: `tests/e2e/login-rate-limit.test.ts` 가
   `429` 를 검증.

### Tier 2 — 깊은 안전 큐 (밤을 채움)

2. `workflows-coSherpa/docs/findings/<UTC>-<slug>.md` (partial, P2) — per-file 마이그레이션
...
5. 무진전 → 사다리(맥락 보강 → 접근 전환 → blocker → 다음 target; goal-iteration.md §When You Are Stuck).
...
```bash
bash workflows-coSherpa/scripts/completion-check.sh; echo "exit: $?"   # 0
