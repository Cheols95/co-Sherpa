...
- **Issue tracker = local markdown.** PRDs in `workflows-coSherpa/docs/prd/PRD.md`; issues in `workflows-coSherpa/docs/issues/<NNN>-<slug>.md`
...
- **Issue frontmatter = machine-readable contract.** Each issue opens with YAML frontmatter carrying
  ...
  `workflows-coSherpa/goals/AGENTS.md` §Risk tier**; omitted = unclassified, NOT mechanical). `/to-issues` emits both; the
...
- **Domain docs = single-context:** one `CONTEXT.md` at repo root + `workflows-coSherpa/docs/adr/`.
- **Phase 1→2 bridge:** `/build` reads `workflows-coSherpa/docs/issues/*.md` (or `workflows-coSherpa/docs/prd/PRD.md`) and writes
  `workflows-coSherpa/goals/<n>-<name>.{md,gates.sh,next-task.sh}` contracts. On the **first** conversion it **replaces
  the `workflows-coSherpa/goals/0-example.*` placeholder triplet** (a teaching example, not a real goal; see the Bootstrap
  rule in `workflows-coSherpa/goals/AGENTS.md`). Goal `<n>` is an ordering label, not a 1:1 map to issue `NNN`; link a
  goal to its issue by slug (`workflows-coSherpa/goals/<n>-<slug>`).
- **FCG invariants:** gates are immutable (fix code, not gates); `workflows-coSherpa/.state/` is gitignored; in loop mode
  never terminate early (stuck → climb the bounded escalation ladder, then log a blocker and move on —
  full ladder: `workflows-coSherpa/guidelines/goal-iteration.md` §When You Are Stuck).

## Spec authority (which doc wins on conflict)

- **Current contract = only what `workflows-coSherpa/docs/spec/INDEX.md` lists** (schema, API contract, public types in
  `workflows-coSherpa/docs/spec/`). This is the single source of truth for implementation. On any conflict, it overrides
  PRD and ADRs.
- **Read order for implementation:** `AGENTS.md` → `CONTEXT.md` (glossary) → `workflows-coSherpa/docs/spec/INDEX.md` →
  the contract docs it points to → latest accepted ADRs → code & tests.
- **PRD (`workflows-coSherpa/docs/prd/`) = original intent, not current state.** Don't treat it as the live spec.
...
- **Overloaded terms — qualify when ambiguous.** "gate" = the machine `.gates.sh` goal check or the
  ...
  gate. "contract" = the `workflows-coSherpa/docs/spec/` interface vs the goal triplet vs issue frontmatter.
