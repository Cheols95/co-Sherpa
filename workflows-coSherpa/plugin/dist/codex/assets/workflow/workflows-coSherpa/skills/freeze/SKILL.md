---
name: freeze
description: "닫힌 기능을 한 번의 호출로 계약·PRD·이슈·graph-lint까지 동결하는 frozen 번들 (Phase 1 → 2 동결). 호출 자체가 유일한 인간 승인점이다. '/freeze', '동결', '기능 동결', '계약 고정', 'freeze 기능X', concept가 '기능 X 닫힘 — /freeze ready' 선언한 직후 활성화. concept으로 결정 체크리스트가 닫힌 기능에만 쓴다."
---
...
- 동결 단위 = **기능 1개**(tracer-bullet). 기능 A를 동결하는 동안 기능 C는 계속 concept 진행 가능.

## 전 단계 공통 규율 — frozen mode
...
4관점·근거 전문은 `workflows-coSherpa/docs/concept/AGENTS.md` §Pre-freeze intent-audit(**동결 전 필독**).

발견이 있으면 **중단** → 사용자 질문으로 종결 → checklist 갱신 후 freeze 재호출(닿은 슬롯만 1회 재walk, 또 나오면 escalate — 열린 루프 금지).

> 이것이 `to-spec` §"동결 전 게이트"를 freeze로 끌어올린 것 — ③에서 `to-spec`을 frozen으로 부를 때 그
...
`workflows-coSherpa/docs/spec/INDEX.md` 존재 여부로 분기한다:
...
출처는 방금 고정한 `workflows-coSherpa/docs/spec/`다.

## ⑤ graph-lint (하드 게이트)

```bash
bash workflows-coSherpa/scripts/issues-graph-check.sh
```
