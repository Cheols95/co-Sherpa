...
| 문서 | 담는 것 | 누가 만드나 |
|---|---|---|
| `flow.md` | 사용 흐름·유스케이스·상태머신·시퀀스 (이용자 관점 시나리오 맵, 엣지 포함) | **concept가 checklist와 함께** 생성·상호검증 |
| `screens.md` | 화면/UI 설계, 와이어프레임 | 필요 시 직접 |
| `architecture.md` | 코드 모듈 경계·배포 경계·의존 방향. 결정 자체는 `workflows-coSherpa/docs/adr/` | **freeze ③(spec 생성)** 이 남김 |

- `flow.md`는 concept가 checklist와 함께 만들고 상호검증한다(`workflows-coSherpa/docs/concept/AGENTS.md` §use-case map).
  freeze가 이걸 `workflows-coSherpa/docs/spec/service-flow.md` `## Use-cases`로 굳힌다.
- `architecture.md`는 freeze의 spec 생성 단계가 남긴다 — service-flow.md 토폴로지(Groups·Components)의 소스.
  사람이 보는 concept 문서를 checklist+flow.md로 좁히려 구조 문서는 freeze로 미룬 것.
- `/to-spec`이 계약을 추출할 때 그 밖의 비계약 내용을 여기로 분류한다. 직접 작성도 가능하다.
- 아키텍처 발굴(기존 코드 deepening)은 `/improve-codebase-architecture`가 맡는다.
