# workflows-coSherpa/goals/EXAMPLE.md — 변환 정답지 (issue → 3파일 goal 계약)

> **이 파일은 goal이 아니라 문서다.** discovery rule(이름이 숫자로 시작하거나
> `_meta.md`인 파일만 goal로 인식 — `workflows-coSherpa/goals/AGENTS.md`)이 무시하므로 체인·게이트에
> 영향이 없고, `0-example`과 달리 첫 변환 후에도 **영구히 남는다**.
>
> **역할 분담** — `workflows-coSherpa/goals/0-example.*` = *실행되는* 교육용 placeholder(새 템플릿에서
> 체인이 green 되는 걸 관찰; 첫 계약화가 삭제). 이 문서 = *계약화 품질의 기준*
> (issue/PRD 계약화 전, finding을 goal로 promote하기 전에 1회 참조).
>
> 규약 본체는 `workflows-coSherpa/goals/AGENTS.md` · `workflows-coSherpa/docs/goal-design.md` §1·§1.5 — 이 문서는 예시일
...
## 입력 — 가상의 이슈 `workflows-coSherpa/docs/issues/003-cancel-booking.md`
...
## 출력 1 — `workflows-coSherpa/goals/3-cancel-booking.md` (미션)
...
취소 규칙이 계약(workflows-coSherpa/docs/spec/api-contract.md §cancel)대로 강제된다:
confirmed 상태에서만 취소되고, 시작 시각이 지난 예약은 409로 거부되며,
취소된 예약의 슬롯은 검색에 재노출된다.

## Completion Conditions

1. Every cancellation rule in `workflows-coSherpa/docs/spec/api-contract.md` §cancel
...
- 규칙 집합: `grep -oE 'CANCEL-R[0-9]+' workflows-coSherpa/docs/spec/api-contract.md`
- 상태 집합: `workflows-coSherpa/docs/spec/domain-types.md` BookingStatus enum

## Verification

    bash workflows-coSherpa/goals/3-cancel-booking.gates.sh
...
- **`risk: RISKY`는 이슈 frontmatter에서 그대로 운반**됐다(엣지·검증 규칙을
  ...
  (`workflows-coSherpa/goals/AGENTS.md` §Risk tier). 이슈에 risk가 없으면 변환 시 휴리스틱으로
...
## 출력 2 — `workflows-coSherpa/goals/3-cancel-booking.gates.sh` (기계 검증, chmod +x)
...
source "$ROOT/workflows-coSherpa/scripts/_gate-cache.sh"

GATE_INPUTS=(
  workflows-coSherpa/docs/spec/api-contract.md
  tests/booking
  workflows-coSherpa/goals/3-cancel-booking.gates.sh
  workflows-coSherpa/goals/3-cancel-booking.md
  workflows-coSherpa/scripts/_gate-cache.sh
)
...
done < <(grep -oE 'CANCEL-R[0-9]+' workflows-coSherpa/docs/spec/api-contract.md | sort -u)
...
echo "[3.3] gate rigor"
if bash "$ROOT/workflows-coSherpa/scripts/check-gate-rigor.sh" "$ROOT/workflows-coSherpa/goals/3-cancel-booking.md" >/dev/null 2>&1; then
  echo "    [PASS]"
else
  echo "    [FAIL]"
  bash "$ROOT/workflows-coSherpa/scripts/check-gate-rigor.sh" "$ROOT/workflows-coSherpa/goals/3-cancel-booking.md" | sed 's/^/      /'
  PASS=false
fi
...
- **게이트 ≠ convention police.** "취소 로직이 옳은가"는 테스트가 검증한다 —
  ...
  테스트 소유, gate에서 빼라 (`workflows-coSherpa/docs/goal-design.md` §1.5).
...
## 출력 3 — `workflows-coSherpa/goals/3-cancel-booking.next-task.sh` (힌트, chmod +x)
...
if ! compgen -G "tests/booking/*cancel*" >/dev/null; then
  echo "TASK: RED -- write the first failing cancellation test (spec: workflows-coSherpa/docs/spec/api-contract.md #cancel)"
elif ! bash "$ROOT/workflows-coSherpa/goals/3-cancel-booking.gates.sh" >/dev/null 2>&1; then
  echo "TASK: GREEN -- make the remaining cancellation rules pass, then re-run the gate"
else
  echo "TASK: goal 3-cancel-booking is green -- run workflows-coSherpa/scripts/active-check.sh to advance"
fi
```
...
(`workflows-coSherpa/guidelines/goal-iteration.md` §Designing next-task hints).
...
5. 횡단 검사(lint/typecheck/test/build)는 이 goal이 아니라 `_meta` 소관 —
   **첫 변환이면 `workflows-coSherpa/goals/_meta.gates.sh`의 `META_CHECKS`/`GATE_INPUTS`를 스택에 맞게 채우고
   `bash workflows-coSherpa/goals/_meta.gates.sh`로 1회 실행 검증**(build 스킬 계약화 절차). 안 채우면 _meta가
   vacuous-pass = 회귀 안전망 부재 → `diagnose.sh`가 경고.
6. Forbidden actions에 scope 경계 명시.
7. gate/`_meta`가 테스트 러너를 호출하면 디렉토리가 아니라 파일/글롭 인자 —
   디렉토리 발견 의미는 런타임 버전마다 달라 wiring-red 유발(`workflows-coSherpa/docs/goal-design.md` §1.5).
8. 첫 변환이면 `workflows-coSherpa/goals/0-example.*` 삭제(부트스트랩 규약) — 이 `EXAMPLE.md`는
   문서라 삭제 대상이 아니다.
