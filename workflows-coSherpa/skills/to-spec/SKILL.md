---
name: to-spec
description: "workflows-coSherpa/docs/spec/가 아직 없을 때 PRD/ADR을 계약 스펙으로 처음 만드는(0→1 생성) 생성기 — 이미 workflows-coSherpa/docs/spec/가 있고 드리프트만 맞추는 건 spec-sync 영역(이건 생성 전용). Phase 1 마무리(계약 고정) 산출물로, accepted ADR + PRD + CONTEXT 용어를 읽어 구현이 단일 진실원천으로 최우선 참조하는 계약문서(data-schema·api-contract·domain-types 등)와 workflows-coSherpa/docs/spec/INDEX.md를 만든다. '/to-spec', '스펙 만들어줘', '계약문서 생성', 'workflows-coSherpa/docs/spec 초기화', 'PRD/ADR을 계약 스펙으로', '스펙 문서 작성' 요청 시 활성화."
---

# to-spec — 계약 스펙 생성기 (Phase 1 마무리 · 계약 고정)

`/concept` 산출 ADR → `to-prd`(PRD)로 **결정이 또렷해진 시점**에,
그 결정을 구현이 따라야 할 **계약문서**로 굳혀 `workflows-coSherpa/docs/spec/`에 처음 생성한다.
(이 계약을 먼저 고정한 뒤 `to-issues`가 그 위에서 작업 슬라이스를 자른다.)
`AGENTS.md`의 §Spec authority에 따라 `workflows-coSherpa/docs/spec/INDEX.md`가 가리키는 문서가 **PRD·ADR보다 우선하는
...
- `/to-issues`·`/build` 변환·`/handoff` **전**이 정위치. 구현 모델이 `workflows-coSherpa/docs/spec/INDEX.md`를 최우선으로 읽기 때문.
- 이미 `workflows-coSherpa/docs/spec/`가 있고 코드와 어긋난 걸 맞추는 거라면 → 이 스킬이 아니라 `spec-sync`.

## 계약 vs 설계 — 무엇을 workflows-coSherpa/docs/spec/에 넣나 (핵심 경계)

**계약(contract) = 코드가 반드시 따라야 하는 인터페이스. PRD/ADR을 override. → `workflows-coSherpa/docs/spec/`**

| 계약문서 | 파일 | 담는 것 |
|---|---|---|
| 데이터 스키마 | `workflows-coSherpa/docs/spec/data-schema.md` | 테이블·엔티티·관계·제약·인덱스·마이그레이션 규칙 |
| API 계약 | `workflows-coSherpa/docs/spec/api-contract.md` | 엔드포인트, 요청/응답 페이로드, 상태코드, **에러 응답 포맷** |
...
각 `workflows-coSherpa/docs/spec/*.md`에서 **내용이 채워진 항목**은 아래를 통과해야 한다. 가드레일대로 정당하게
...
각 `workflows-coSherpa/docs/spec/*.md` 상단:
```markdown
---
title: <문서명>
created: <YYYY-MM-DD>
...
## `workflows-coSherpa/docs/spec/INDEX.md` 형식
