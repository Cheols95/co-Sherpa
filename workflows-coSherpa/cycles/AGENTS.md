# `workflows-coSherpa/cycles/` — Working Protocol for cycle-driver documents

## What this directory is

`workflows-coSherpa/cycles/` holds **loop-driver prompts** — the documents you hand to an
autonomous coding agent in infinite-loop mode:

```
<your /goal-style command> workflows-coSherpa/cycles/260526-01-overnight-findings-sweep.md
의 내용을 모두 완수할 때까지 작업해줘.
```

A cycle document describes **one work session**: it picks unresolved items
from `workflows-coSherpa/docs/findings/`, orders them by priority/dependency, and drives a TDD
loop that closes them — promoting some into `workflows-coSherpa/goals/<n>-*` harness goals,
...
|              | `workflows-coSherpa/goals/<n>-*.md`                                          | `workflows-coSherpa/cycles/YYMMDD-NN-*.md`                |
| ------------ | -------------------------------------------------------- | -------------------------------------- |
...
The harness only scans the top of `workflows-coSherpa/goals/`, so this folder is **never a
...
```
workflows-coSherpa/cycles/<YYMMDD>-<NN>-<slug>.md
```
...
```
workflows-coSherpa/cycles/260523-01-overnight-findings-closure.md
workflows-coSherpa/cycles/260524-01-post-review-findings-closure.md
workflows-coSherpa/cycles/260526-01-overnight-findings-sweep.md
```
...
Generator meta-prompt: `workflows-coSherpa/prompts/cycle-generate.md`. Minimum structure:
...
7. **Termination / verification** — the commands that confirm it's
   ...
   is the cheapest moment to reconcile `workflows-coSherpa/docs/spec/`.

---

## Authoring rules

Read before authoring: `workflows-coSherpa/docs/goal-design.md` (especially §1.5, §5),
`workflows-coSherpa/guidelines/goal-iteration.md`, your commit convention, and
`workflows-coSherpa/docs/findings/AGENTS.md`.
...
- **Promote sparingly.** Only promote a finding to a goal when all three
  ...
  Minimal-gate rules: `workflows-coSherpa/docs/goal-design.md §1.5`.
- **Design for unattended (overnight) runs.** Put deep, safe queues
  ...
  steps and counts: `workflows-coSherpa/guidelines/goal-iteration.md` §When You Are Stuck.
- **Output discipline (loop mode).** Speak only at a real question, a
  ...
  are already visible). Full rule: `workflows-coSherpa/skills/build/SKILL.md` §출력 규율.

---

## Lifecycle

1. **Generate** — author a new cycle doc with `workflows-coSherpa/prompts/cycle-generate.md`.
2. **Run** — hand `workflows-coSherpa/cycles/<file>` to the infinite-loop agent.
...
(`workflows-coSherpa/docs/findings/<file>.md`, `workflows-coSherpa/goals/<n>-<name>.md`, `workflows-coSherpa/cycles/<file>.md`) —
no `./` or `../`.
