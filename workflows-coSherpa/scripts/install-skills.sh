...
# in-repo backup (workflows-coSherpa/skills/) into the global skill homes, and (re)generate the
...
# workflows-coSherpa/skills/ directory is the SINGLE SOURCE for all workflow skills (inert
# backup -- Claude/Codex never load it directly), and this script deploys it:
#
#   workflows-coSherpa/skills/<all>   -> ~/.claude/skills/   (Claude)
#   workflows-coSherpa/skills/<all>   -> ~/.codex/skills/    (Codex / GPT) -- FULL PARITY
...
#   bash workflows-coSherpa/scripts/install-skills.sh --force    # overwrite existing (deploy updates)
#   CLAUDE_SKILLS_DIR=/path  bash workflows-coSherpa/scripts/install-skills.sh   # custom Claude home
#   CODEX_SKILLS_DIR=/path   bash workflows-coSherpa/scripts/install-skills.sh   # custom Codex skills home
#   CODEX_PROMPTS_DIR=/path  bash workflows-coSherpa/scripts/install-skills.sh   # custom Codex prompts home
#
# Idempotent; re-run with --force after editing any skill in workflows-coSherpa/skills/.
...
echo "=== install-skills: workflows-coSherpa/skills/ -> $CLAUDE_DST (Claude) ==="
install_all "$CLAUDE_DST" "claude"
echo

echo "=== install-skills: workflows-coSherpa/skills/ -> $CODEX_DST (Codex / full parity) ==="
...
machine. Repo assets (workflows-coSherpa/scripts/, workflows-coSherpa/goals/ conventions,
workflows-coSherpa/dashboard/, ...) are per-project -- propagate those with
workflows-coSherpa/scripts/update-workflow.sh instead.
