...
> Test Plan → TDD → Verify → Record → Commit), `workflows-coSherpa/docs/state/*` 를 어떻게
> 다루는지, gate 를 어떻게 설계하는지 같은 **운영 매뉴얼**은
> `workflows-coSherpa/guidelines/goal-iteration.md` 에 있다. goal 루프로 작업하는 에이전트는
> 그 파일을 먼저 읽어야 한다.

## 핵심 아이디어

- 미션은 `workflows-coSherpa/goals/` 아래의 **버전드 goal 스택**으로 표현된다.
...
| 디렉토리         | 무엇인가                          | 검증     | 수명           |
| ---------------- | --------------------------------- | -------- | -------------- |
| `workflows-coSherpa/docs/findings/` | 부채/통찰 **큐** (out-of-scope 발견) | 없음     | promote/close 까지 |
| `workflows-coSherpa/cycles/`        | 무한 루프에 넘기는 **세션 프롬프트** | 없음     | 영구 (이력)    |
| `workflows-coSherpa/goals/`         | gate 로 검증되는 **영속 invariant** | `.gates.sh` | 영구           |

흐름: iteration 중 발견한 out-of-scope 부채는 `workflows-coSherpa/docs/findings/` 에 큐잉
된다. 사람이 `workflows-coSherpa/cycles/` 문서를 써서 루핑 에이전트에 넘기면, 에이전트가
finding 들을 우선순위대로 닫는다 — 일부는 `workflows-coSherpa/goals/` 로 promote 해 영속
...
커뮤니케이션·상호학습**이다. 회귀·계약 위반의 기계 검증은 이미 `workflows-coSherpa/goals/` 의
...
중 발견과 **동등한 finding 의 1급 출처**다 (`workflows-coSherpa/docs/findings/AGENTS.md` 의
...
### `workflows-coSherpa/goals/` — 미션 스택
...
### `workflows-coSherpa/scripts/` — goal-agnostic 하네스

| 파일                  | 역할                                                                                                  |
| --------------------- | ----------------------------------------------------------------------------------------------------- |
| `diagnose.sh`         | 매 iteration 첫 단계. git 상태, active goal, 열린 finding, blocker 를 한 번에 출력 (read-only).        |
| `completion-check.sh` | goal 들을 번호순/병렬로 돌며 각 `<n>-*.gates.sh` 실행. **첫 실패 goal 을 `workflows-coSherpa/.state/active-goal` 에 기록**. |
| `active-check.sh`     | active goal 의 gate + rigor sweep 만 (~5–30 s). green 이면 completion-check 로 exec 해 포인터 전진.    |
| `next-task.sh`        | `workflows-coSherpa/.state/active-goal` 읽고 해당 goal 의 `next-task.sh` 로 dispatch.                                     |
| `update-state.sh`     | `workflows-coSherpa/docs/state/progress.md`, `next-task.md` 를 git + goal 상태로 재생성.                                  |
...
### `workflows-coSherpa/.state/` — 하네스의 휘발성 상태 (gitignore)

- `active-goal` — 첫 실패 goal 경로, 또는 `ALL_DONE`.
- `gate-cache/` — goal 별 input fingerprint.

### `workflows-coSherpa/docs/state/` — 에이전트의 스크래치패드
...
```
bash workflows-coSherpa/scripts/diagnose.sh              # cheap: workflows-coSherpa/.state/active-goal 만 읽고 표시
   └─ next-task.sh
        └─ workflows-coSherpa/.state/active-goal 읽음
        └─ workflows-coSherpa/goals/<active>.next-task.sh exec → "다음 할 일" 출력

# 에이전트가 출력대로 RED 테스트 → 커밋 → GREEN 코드 → 커밋 (TDD 룰)

bash workflows-coSherpa/scripts/update-state.sh          # workflows-coSherpa/docs/state/* 갱신
bash workflows-coSherpa/scripts/active-check.sh          # 활성 goal 만 검사 + rigor sweep
   └─ rigor 실패 → exit 1 (의미 drift)
   └─ 활성 goal 아직 fail → exit 1, workflows-coSherpa/.state/active-goal 유지
...
cheat 방지. `workflows-coSherpa/scripts/check-gate-rigor.sh` 가 이걸 메타-검증한다.
...
| 유스케이스 | `find workflows-coSherpa/docs/usecases -name 'UC-*.md'`                       |
...
3. **구조 앵커** — 후속 goal 이 routing 신호로 쓰는 문서/파일의 존재. (이건 *빌드 하네스가 다음
   ...
   출력물]의 실존은 여기가 아니라 `workflows-coSherpa/guidelines/품질점검.md` §5 가 본다.)
...
스크립트·deep gate(별도 cadence)·`workflows-coSherpa/guidelines/품질점검.md` §5 로 검증한다 (exit 0 ≠ 작동). deep
...
`workflows-coSherpa/scripts/red-first-check.sh` 가 변환 직후 이런 "assertion 이 아니라 wiring 에러로
인한 red" 를 경고하지만, 애초에 파일/글롭으로 호출하는 게 1차 방어다.

### 2. 첫 실패 goal = 작업 대상

`completion-check.sh` 가 goal 들을 번호순으로 돌면서 첫 실패만
`workflows-coSherpa/.state/active-goal` 에 쓰고, 이후 모든 도구가 그 한 파일을 신호로 사용.
새 goal 을 추가하면 자동으로 흐름이 거기로 흘러간다.

### 3. 자체 검사만 (회귀는 orchestrator 가 담당)

각 goal 의 `.gates.sh` 는 그 goal 의 **자체** invariant 만 검사한다 —
이전 goal 에 대한 회귀 검사는 `workflows-coSherpa/scripts/completion-check.sh` 가 단독으로
책임진다.

- `bash workflows-coSherpa/goals/3-foo.gates.sh` 를 단독 실행하면 goal 3 만 검사한다. goal
  0/1/2 가 깨졌는지는 모름.
- "전체 체인 green" 을 보려면 `bash workflows-coSherpa/scripts/completion-check.sh` 를
  돌린다. 모든 goal 의 gate 를 병렬로 띄우고 하나라도 fail 이면 첫 실패
  goal 을 `workflows-coSherpa/.state/active-goal` 에 기록.
...
`workflows-coSherpa/scripts/completion-check.sh` 는 goal 당 정확히 1번 실행한다. 그 위에:

### 1. Per-goal cache

각 goal 의 gate 스크립트는 `workflows-coSherpa/scripts/_gate-cache.sh` 를 source 한다.
...
- gate 성공 시 fingerprint 를 `workflows-coSherpa/.state/gate-cache/<goal-name>` 에 저장.
- **실패하면 캐시를 저장하지 않는다** — 다음 실행에서 반드시 재실행.
- `workflows-coSherpa/.state/` 는 `.gitignore` 라 커밋되지 않음.
...
```
rm -rf workflows-coSherpa/.state/gate-cache             # 전체 캐시 버스트
rm    workflows-coSherpa/.state/gate-cache/0-init       # 한 goal 만
GATES_NO_CACHE=1 bash workflows-coSherpa/goals/0-init.gates.sh   # 일회성 우회
```
...
`workflows-coSherpa/.state/active-goal` 이 `ALL_DONE` 이면 모든 gate 가 통과한 상태. 새
작업이 필요하면 세 파일 세트를 추가한다:

```
workflows-coSherpa/goals/4-<name>.md            # 미션
workflows-coSherpa/goals/4-<name>.gates.sh      # 머신 검증 (chmod +x)
workflows-coSherpa/goals/4-<name>.next-task.sh  # 다음 액션 hint (chmod +x)
```
...
> `workflows-coSherpa/guidelines/goal-iteration.md`를 읽어 iteration 프로토콜을 확인할 것.
```
...
2. 이전 게이트의 **검사 로직 자체**를 바꿔야 하는가? → 케이스 (b).
   `workflows-coSherpa/docs/findings/*.md` 에 큐잉하고 별도 PR 로 분리.
