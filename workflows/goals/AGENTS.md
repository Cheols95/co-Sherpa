# `workflows/goals/` — Working Protocol for the mission stack

`workflows/goals/` is the **mission stack**: the versioned, machine-verified
...
Read `workflows/docs/goal-design.md` (design) and `workflows/guidelines/goal-iteration.md`
...
**Discovery rule.** The harness treats a markdown file in `workflows/goals/` as a
...
`workflows/goals/0-example.*` is a **teaching placeholder**, not part of the real
chain. The **first** `build` mode-B conversion (issues/PRD → goals)
MUST delete the `workflows/goals/0-example.{md,gates.sh,next-task.sh}` triplet as it
...
the chain go green, and `workflows/scripts/reset-for-new-project.sh` preserves it
...
```
workflows/goals/<n>-<name>.md            # mission
workflows/goals/<n>-<name>.gates.sh      # machine verification (chmod +x)
workflows/goals/<n>-<name>.next-task.sh  # next-action hint (chmod +x)
```

Before authoring your first real goal — or converting issues in mode B —
read `workflows/goals/EXAMPLE.md` once: an annotated issue→three-file conversion
...
Before writing it, run the **self-audit** in `workflows/docs/goal-design.md`
...
- **Risk heuristic (a floor, judged per slice) — single source.** `RISKY` if the slice names an
  ...
  `workflows/docs/spec/` contract surface. `NONE` if it has no behavioral surface (workflows/docs/config/pure scaffold).
...
- **RISKY close-out review.** When a `RISKY` goal's gates go green, **before advancing**:
  ...
  Findings go to `workflows/docs/findings/` (finding 규약, file:line evidence) — they do **NOT**
...
Every gate should source `workflows/scripts/_gate-cache.sh`, declare `GATE_INPUTS`,
