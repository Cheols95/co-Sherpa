...
# in the template's workflows/workflow-manifest.txt, read at the template's HEAD --
...
#   bash workflows/scripts/update-workflow.sh --from <template-path-or-git-url>   # dry-run report
#   bash workflows/scripts/update-workflow.sh --apply                             # apply NEW+UPDATE
#   bash workflows/scripts/update-workflow.sh --set-baseline                      # mark reconciled
...
#   cd <project> && bash <template>/workflows/scripts/update-workflow.sh --from <template>
...
# -- project-root guard ------------------------------------------------------
if [ ! -f "$ROOT/AGENTS.md" ] && [ ! -d "$ROOT/workflows/goals" ]; then
  echo "[FAIL] run from the project root (no AGENTS.md / workflows/goals/ here): $ROOT"
  exit 2
fi
...
manifest_raw=$(git -C "$TPL" show "$TPL_HEAD:workflows/workflow-manifest.txt" 2>/dev/null) || {
  echo "[FAIL] template HEAD carries no workflows/workflow-manifest.txt -- commit it in the template first."
  exit 2
}
...
if [ "$pending" -gt 0 ]; then
  ...
  echo "  When everything is reconciled:  bash workflows/scripts/update-workflow.sh --set-baseline"
fi
