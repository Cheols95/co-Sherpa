...
항상 진단부터:
```bash
bash workflows/scripts/diagnose.sh        # git/active-goal/열린 finding/blocker (read-only)
```
그 다음 TDD 루프:
```bash
bash workflows/scripts/active-check.sh    # active goal gate + rigor sweep. green이면 자동 completion-check
bash workflows/scripts/completion-check.sh # 전체 goal gate 병렬, 첫 실패를 workflows/.state/active-goal에 기록
bash workflows/scripts/update-state.sh    # workflows/docs/state/{progress,next-task}.md 재생성
```
...
```
workflows/goals/<n>-<name>.md            # Mission: 'done' 조건을 prose로 (universal claim 사용 시 enumerate)
workflows/goals/<n>-<name>.gates.sh      # 기계 검증 (chmod +x). "every X"면 source of truth에서 X를 enumerate
workflows/goals/<n>-<name>.next-task.sh  # 다음 액션 힌트 (chmod +x, 절대 gate 아님)
```
- **변환 전 1회 참조**: `workflows/goals/EXAMPLE.md`(이슈→3파일 모범 변환 정답지, 안티패턴 주석 포함)가 프로젝트에 있으면 먼저 읽는다. 구버전 템플릿엔 없을 수 있다 — 그러면 `workflows/goals/AGENTS.md` 규약만으로 진행.
- **첫 변환**이면 `workflows/goals/0-example.*` 교육용 placeholder triplet을 제거한다(`workflows/goals/AGENTS.md` 부트스트랩 규약 — 안 지우면 예제가 실제 체인에 영구 잔존). `workflows/goals/EXAMPLE.md`는 goal이 아니라 문서이므로 **삭제 대상이 아니다**.
- **risk 운반**: 이슈 frontmatter `risk: RISKY|MECHANICAL|NONE`을 goal `.md` frontmatter로 그대로 옮긴다.
  이슈에 없으면 변환 시 휴리스틱으로 판정(`workflows/goals/AGENTS.md` §Risk tier) — **미지정은
...
- **횡단 불변식 = `_meta` 세트 — 첫 변환에서 이빨을 켠다 (자동, 사용자 개입 불필요).**
  ...
  (4) 서비스가 뜨면 `workflows/scripts/smoke.sh`(부팅→헬스체크→2xx→teardown)를 만든다(=`_meta`에 런타임 스모크 자동 배선),
  (5) `bash workflows/goals/_meta.gates.sh`를 1회 실행해 명령이 실제로 돌고 통과하는지 확인한다(wiring 오류 조기 발견).
  안 채우면 회귀 안전망이 빈 총인 채 남는다 — `diagnose.sh`가 매 진단에서 경고한다.
- 각 gate는 `workflows/scripts/_gate-cache.sh`를 source하고 `GATE_INPUTS`를 선언하며, 끝에 `check-gate-rigor.sh`로 자기 `.md` 정합성을 검사.
...
- **red-first 위생검사 (변환 직후 1회)**: 3파일을 다 만든 직후, 구현 전 코드에서
  `bash workflows/scripts/red-first-check.sh`로 새 goal 게이트가 **전부 red인지** 확인한다. 구현 전
...
3. **무인 운영 규율**: 무진전이면 사다리(맥락 보강 → 접근 전환 → blocker 기록 후 다음 target)를 밟고 **절대 조기 종료 금지** — 모든 in-scope가 resolved/partial이고 체인이 green일 때만 종료. 사다리 단계·횟수는 `workflows/guidelines/goal-iteration.md` §When You Are Stuck.
4. out-of-scope로 명시된 항목은 발견해도 건드리지 않는다.
5. **종료 보고에 `/spec-sync` 1회 권고를 포함**한다 — 사이클 직후가 계약(workflows/docs/spec) 정합의 최적 시점(근거: `workflows/cycles/AGENTS.md` §"A cycle document MUST contain" — Termination/verification 항).
...
`workflows/.state/`(active-goal 포인터 + gate 캐시)는 런타임 생성물이며 **gitignore 대상**이다.
