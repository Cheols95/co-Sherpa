# `workflows/docs/findings/` — Working Protocol
...
```
workflows/docs/findings/<UTC-created-at>-<slug>.md
```
...
| `was`          | repo-rooted path          | no       | —       | Previous goal path (`workflows/goals/<n>-<name>.md`) when a promoted goal dissolved back into the queue.                         |
...
3. **Promote** — when a finding warrants its own goal, author
   `workflows/goals/<n>-<name>.md` and have its `## Why This Goal Exists` section
...
5. **Archive** — append-only logs (`kind: append-only-log`) that exceed
   ...
   `workflows/docs/archive/findings/<slug>-<YYYY>-Q<n>.md`.
...
(`workflows/docs/findings/<file>.md`, `workflows/goals/<n>-<name>.md`, `src/...`) — not `./`
...
(`workflows/docs/findings/<file>.md`). Bare globs (`workflows/docs/findings/*.md`) are reserved
