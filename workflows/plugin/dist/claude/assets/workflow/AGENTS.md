...
- Full workflow (human reference): `Workflow_Guideline_v1.html`. FCG mechanics: `workflows/docs/fcg-system.md`.
...
- **Issue tracker = local markdown.** PRDs in `workflows/docs/prd/PRD.md`; issues in `workflows/docs/issues/<NNN>-<slug>.md`
...
- **Issue frontmatter = machine-readable contract.** Each issue opens with YAML frontmatter carrying
  ...
  `workflows/goals/AGENTS.md` §Risk tier**; omitted = unclassified, NOT mechanical). `/to-issues` emits both; the
...
- **Domain docs = single-context:** one `CONTEXT.md` at repo root + `workflows/docs/adr/`.
- **Phase 1→2 bridge:** `/build` reads `workflows/docs/issues/*.md` (or `workflows/docs/prd/PRD.md`) and writes
  `workflows/goals/<n>-<name>.{md,gates.sh,next-task.sh}` contracts. On the **first** conversion it **replaces
  the `workflows/goals/0-example.*` placeholder triplet** (a teaching example, not a real goal; see the Bootstrap
  rule in `workflows/goals/AGENTS.md`). Goal `<n>` is an ordering label, not a 1:1 map to issue `NNN`; link a
  goal to its issue by slug (`workflows/goals/<n>-<slug>`).
- **FCG invariants:** gates are immutable (fix code, not gates); `workflows/.state/` is gitignored; in loop mode
  never terminate early (stuck → climb the bounded escalation ladder, then log a blocker and move on —
  full ladder: `workflows/guidelines/goal-iteration.md` §When You Are Stuck).

## Spec authority (which doc wins on conflict)

- **Current contract = only what `workflows/docs/spec/INDEX.md` lists** (schema, API contract, public types in
  `workflows/docs/spec/`). This is the single source of truth for implementation. On any conflict, it overrides
  PRD and ADRs.
- **Read order for implementation:** `AGENTS.md` → `CONTEXT.md` (glossary) → `workflows/docs/spec/INDEX.md` →
  the contract docs it points to → latest accepted ADRs → code & tests.
- **PRD (`workflows/docs/prd/`) = original intent, not current state.** Don't treat it as the live spec.
- **ADR (`workflows/docs/adr/`) = decision history, one decision per file.** A `superseded by ADR-NNNN` ADR is
  history, not a basis for implementation. ADRs stay in place — don't archive to read out of context.

## Phase 1 elicitation (planning gate) — full rules in `workflows/docs/design/AGENTS.md`

Sequence: `/concept` (account the decision surface in `workflows/docs/design/checklist.md` until every slot closes)
...
intent-audit, freeze-is-one-way) are the single source in `workflows/docs/design/AGENTS.md`** — `/concept` and
`/freeze` read it before working. Checklist format: `workflows/docs/design/README.md`.

## Gate validity (Phase 1→2) — full rules in `workflows/goals/AGENTS.md` §Gate validity
...
`workflows/goals/AGENTS.md` §Gate validity** — `/build` reads it when writing gates.
...
`workflows/goals/<n>-*.gates.sh` instead.
...
- **Overloaded terms — qualify when ambiguous.** "gate" = the machine `.gates.sh` goal check or the
  ...
  gate. "contract" = the `workflows/docs/spec/` interface vs the goal triplet vs issue frontmatter.
