...
`completion-check.sh` recognizes `workflows/goals/_meta.md` and launches it first.
If it fails, `workflows/.state/active-goal` records `_meta` and `next-task.sh`
dispatches to `workflows/goals/_meta.next-task.sh`.

## The Goal (adapt to your stack)

The following hold for the whole repo. Edit `META_CHECKS` in
`workflows/goals/_meta.gates.sh` to match your toolchain:
...
When the project grows a runnable service, create **`workflows/scripts/smoke.sh`**:
...
No `workflows/scripts/smoke.sh` = an explicit "this project has no runtime
component" decision — record that in `workflows/docs/spec/`, don't leave it implicit.
