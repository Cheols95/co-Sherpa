...
> conversion (mode B) writes `workflows/goals/<n>-<name>.{md,gates.sh,next-task.sh}`
> from your spec and **removes this triplet** as it does so (the
> `workflows/goals/AGENTS.md` bootstrap rule; you can also delete it by hand).

## Mission

Every shell script under `workflows/scripts/` that is meant to be run directly
(i.e. not a sourced helper prefixed with `_`) begins with a `#!` shebang.

## Completion Conditions

1. Every runnable `workflows/scripts/*.sh` starts with a `#!` line.
...
```
bash workflows/goals/0-example.gates.sh
