# workflows/guidelines/goal-iteration.md — Goal-Looping Workflow

이 문서는 `workflows/docs/goal-design.md` 의 autonomous build harness 위에서 매
...
cat workflows/docs/state/next-task.md
```
...
`workflows/docs/usecases/`, `workflows/docs/specs/`, 이슈 트래커). 식별할 것: 메인 성공
시나리오, 확장 시나리오, 선·후행 조건, 필요한 엔티티와 관계.

### Phase 3: Test Plan (5–10%)

테스트를 쓰기 전에 계획한다. `workflows/docs/state/test-plan.md` 에 append:
...
> mocking·리팩토링 **노하우**는 `workflows/guidelines/tdd/`(구 tdd 스킬) 참조 — 거기는 "이 루프 안에서 어떻게 잘 쓰나".
...
> `workflows/guidelines/품질점검.md` §5 의 cadence(finding 2개마다/주요 변경 후)로 점검한다 — 좁은 per-goal
> 검사로 끌어오지 않는다(`workflows/docs/goal-design.md` §1.5). 자동화 가능하면 별도 cadence 의 deep
> gate(`GATES_SKIP_DEEP=0`)로 돌리되, 그 검사 라벨은 `workflows/goals/_meta.gates.sh` 의 `DEEP_LABELS_RE` 에
> 매치하도록 달아 빠른 반복(inner-loop)에서 skip 되게 한다.

### Phase 6: Record (5%)

```
bash workflows/scripts/update-state.sh
bash workflows/scripts/active-check.sh
```
...
중요한 걸 발견했으면 `workflows/docs/state/learnings.md` 에 한 줄 append.

`workflows/docs/state/*` 와 `learnings.md` 변경은 TDD 커밋과 섞지 말고 별도 커밋:
...
`.gitignore` 누락 점검, 전체 회귀 필요 시 `bash workflows/scripts/completion-check.sh`.
...
1. **1단 — 맥락 보강 재시도 (1회).** 멈추고 빠진 맥락을 다시 모은 뒤
   ...
   forbidden-actions 재독, `workflows/docs/spec/` 의 해당 계약 슬라이스, 관련
   finding / `learnings.md`, 실패 출력 전문.
2. **2단 — 접근 전환 재시도 (1회).** 그래도 무진전이면 접근 자체를
   바꿔 한 번 더 시도한다: 다른 구현 전략, 다른 테스트 각도, 다른
   문제 분해.
3. **기록.** 그래도 안 되면 `workflows/docs/state/blockers.md` 에 append:
   하려던 것 / **각 단에서 시도한 것과 결과** / 무엇이 잘못되는지.
4. `workflows/scripts/next-task.sh` 로 다른 task 로 이동. fresh context 로 나중에
   복귀.
...
`workflows/docs/state/*` 는 에이전트 관리 스크래치 공간이다.
...
`workflows/goals/<n>-<name>.gates.sh` 가 그 조건을 기계적으로 검증한다. 한 가지
규칙:
...
`workflows/scripts/check-gate-rigor.sh` 가 모든 goal 에 대해 돌며, markdown 에
...
흉내내지 마라.** 근거는 `workflows/docs/goal-design.md §1.5`. 회의적 휴리스틱:
...
```
bash workflows/scripts/diagnose.sh        # active goal 경로 출력
cat workflows/.state/active-goal          # completion-check.sh 가 기록
```
...
| 명령                               | 범위                      | 비용         | 시점                      |
| ---------------------------------- | ------------------------- | ------------ | ------------------------- |
| `bash workflows/goals/<n>-*.gates.sh`        | 한 goal                   | 초~분        | 한 goal 수동 점검         |
| `bash workflows/scripts/active-check.sh`     | active goal + rigor sweep | ~5–30 s      | 매 TDD 사이클 (Phase 6)   |
| `bash workflows/scripts/completion-check.sh` | 모든 goal                 | 1–3 분 (캐시)| pre-push, CI, 수동 sweep  |

단일 goal pass 만으로 TDD 사이클을 끝났다고 선언하지 마라 —
orchestrator 가 계약이다.

지금 `bash workflows/scripts/diagnose.sh` 를 돌리고 그것이 가리키는 active goal
파일을 읽어라.
