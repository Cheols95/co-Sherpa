...
요지: **findings**(out-of-scope 발견 큐, 검증 없음, close까지) → **cycles**(무한 루프 한 세션 프롬프트, 검증 없음, 영구 이력) → **goals**(gate로 검증되는 영속 invariant, goal당 3파일, 영구). 전체 표·각 자산 상세는 `workflows/docs/goal-design.md` §"세 가지 자산: findings → cycles → goals"가 단일 출처.
...
## 오케스트레이터 스크립트 (`workflows/scripts/`)

핵심 7종: `diagnose.sh`(매 iter 첫 단계, read-only 상태 출력) · `next-task.sh`(active goal로 dispatch) · `active-check.sh`(active goal gate + rigor sweep, green이면 completion-check exec) · `completion-check.sh`(모든 goal gate 병렬, 첫 실패를 `workflows/.state/active-goal`에 기록) · `check-gate-rigor.sh`(메타-검증: universal claim ↔ enumerating gate) · `_gate-cache.sh`(input fingerprint memoize, 병렬 안전) · `update-state.sh`(`workflows/docs/state/{progress,next-task}.md` 재생성). 각 스크립트 역할·비용 표는 `workflows/docs/goal-design.md` §`workflows/scripts/`(및 비용 모델 표)가 단일 출처.
...
1. `workflows/docs/goal-design.md` — 왜 이렇게 설계됐는가 (§1, §1.5, §5).
2. `workflows/guidelines/goal-iteration.md` — 한 iteration을 어떻게 도는가.
...
| 작업 | 전역 스킬 | 구동 |
|---|---|---|
| 목표 생성·게이트 루프·사이클 실행 | `/build` | `bash workflows/scripts/{diagnose,active-check,completion-check}.sh` |
| finding 기록·정리 | `/finding` | `workflows/docs/findings/` 문서 작성 |
| cycle 문서 작성 | `/cycle` | `workflows/prompts/cycle-generate.md` + `workflows/docs/findings/` 취합 |
