...
## Phase 1 elicitation (planning gate) — full rules in `docs/grill/AGENTS.md`

Sequence: `/grill` (account the decision surface in `docs/grill/checklist.md` until every slot closes)
...
intent-audit, freeze-is-one-way) are the single source in `docs/grill/AGENTS.md`** — `/grill` and
`/freeze` read it before working. Checklist format: `docs/grill/README.md`.
...
Add one only when a pattern has proven to repeat — don't auto-generate agent/skill rosters (context
bloat hurts performance). Encode tailoring above and in `goals/<n>-*.gates.sh` instead.
